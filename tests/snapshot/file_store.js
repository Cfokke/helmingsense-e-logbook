...
// Node's built-in test runner (node:test) + assert. No external deps.
...
test("appendAndPrune creates file and keeps entries within retention", async () => {
  const store = createFileStore(tmpdir, /* retentionHours */ 24);
  const file = path.join(tmpdir, "signalk_snapshot.json");

  // Build 3 snapshots: now-25h (old), now-2h (recent), now (recent)
  const now = new Date();
  const fmt = (d) => {
  ...
  // Ensure the two kept are the recent and now items
...
test("recovers from corrupt JSON by backing up and starting fresh", async () => {
  ...
  // Append a valid snapshot; store should rename corrupt file and start clean
